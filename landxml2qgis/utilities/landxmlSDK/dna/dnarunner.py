...
class DNARunner:
    ...
    def build_execution_cmd(self, msr_location, stn_location):
        if self.filename is None:
            self.filename = stn_location.parts[-1].replace('.stn', '')
    ...
    def run_dna_via_subprocesses(self, msr_location, stn_location):
        subprocess.call([str(self.dna_import), '-n', self.filename, str(stn_location), str(msr_location),
                         '-r', 'GDA2020'], cwd=self.output_dir)
        if self.geoid_fp is not None:
            if self.convert_stn_heights is True:
        ...
        adj_list = [
            ...
            '--dms-msr-format', self.dms_msr_format, '--stn-coord-types', self.stn_coord_types,
